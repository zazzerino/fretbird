...
(defn add-note [{:keys [context stave note]}]
  (let [stave-note (doto (new (.-StaveNote vexflow)
    ...
    (if accidental
      (.addAccidental stave-note 0
                      (new (.-Accidental vexflow) (name accidental))))
...
(defn stave []
  (let [id "stave"
        ...
        draw            (fn []
                          ...
                          (reset! vexflow-objects (make-vexflow-objects
                                                   {:id "stave"
                                                    :width width
                                                    :height height}))
